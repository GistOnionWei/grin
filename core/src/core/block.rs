...
impl BlockHeader {
	...
	pub fn overage(&self) -> i64 {
		(reward_at_height(self.height) as i64).checked_neg().unwrap_or(0)
	}
	...
	pub fn total_overage(&self, genesis_had_reward: bool) -> i64 {
		...
		((reward_count * reward_at_height(self.height)) as i64).checked_neg().unwrap_or(0)
	}
...
impl Block {
	...
	pub fn validate(
		...
		self.verify_coinbase(self.header.height)?;
	...
	pub fn verify_coinbase(&self, height: u64) -> Result<(), Error> {
		let cb_outs = self
			.body
			.outputs
			.iter()
...

		{
			...
			let secp = secp.lock();
			let over_commit = secp.commit_value(reward(self.total_fees(), height))?;
