...
impl Default for WalletConfig {
	fn default() -> WalletConfig {
		WalletConfig {
			...
			api_listen_port: 1777,
			owner_api_listen_port: Some(WalletConfig::default_owner_api_listen_port()),
			api_secret_path: Some(".api_secret".to_string()),
			node_api_secret_path: Some(".api_secret".to_string()),
			check_node_api_http_addr: "http://127.0.0.1:1773".to_string(),
...
impl WalletSeed {
	...
	pub fn from_file(wallet_config: &WalletConfig, password: &str) -> Result<WalletSeed, Error> {
		...
		if Path::new(seed_file_path).exists() {
			...
			error!(
				"wallet seed file {} could not be opened (libercoin wallet init). \
				 Run \"libercoin wallet init\" to initialize a new wallet.",
				seed_file_path
			);
