	...
	K: Keychain + 'static,
{
	...
	if api_secret.is_some() {
		let api_basic_auth =
			"Basic ".to_string() + &to_base64(&("libercoin:".to_string() + &api_secret.unwrap()));
		let basic_realm = "Basic realm=LibercoinOwnerAPI".to_string();
	...
	pub fn issue_send_tx(
		...
		Box::new(parse_body(req).and_then(move |args: SendTXArgs| {
			...
			let (mut slate, lock_fn) = match result {
				Ok(s) => {
					info!(
						"Tx created: {} libercoin to {} (strategy '{}')",
