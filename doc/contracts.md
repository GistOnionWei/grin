...
This document describes smart contracts that can be setup using Libercoin even
though the Libercoin chain does not support scripting. All these contracts rely
...
document or the Libercoin development team. Most of the credit should be attributed
...
This section is meant as a reminder of some crucial features of the Libercoin chain.
...
built for a Libercoin transaction by taking the above private key `x` to be the sum
...
public key `r*G`, and allows to verify non-inflation for all Libercoin transactions
(and signs the fees).
...
receiving party. As Libercoin outputs are completely obscured by Pedersen
...
Alice wants to pay Bob in libercoins. She starts the transaction building process:
...
Alice has libercoins and Bob has bitcoin. They would like to swap. We assume Bob
...
ready to send her libercoins to Bob if he reveals `x`.

First, Alice sends her libercoins to a multiparty timelock contract with a refund
...
1. As soon as Bob broadcasts the final transaction to get his new libercoins, Alice
