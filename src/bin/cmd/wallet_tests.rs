...
mod wallet_tests {
	...
	use libercoin_wallet;

	use libercoin_wallet::test_framework::{self, LocalWalletClient, WalletProxy};

	use clap::{App, ArgMatches};
	use libercoin_util::Mutex;
	use std::sync::Arc;
	use std::thread;
	use std::time::Duration;
	use std::{env, fs};

	use libercoin_config::GlobalWalletConfig;
	use libercoin_core::global;
	use libercoin_core::global::ChainTypes;
	use libercoin_keychain::ExtKeychain;
	use libercoin_wallet::{LMDBBackend, WalletBackend, WalletConfig, WalletInst, WalletSeed};
	...
	pub fn config_command_wallet(
		...
		config_file_name.push("libercoin-wallet.toml");
		if config_file_name.exists() {
			return Err(libercoin_wallet::ErrorKind::ArgumentError(
				"libercoin-wallet.toml already exists in the target directory. Please remove it first"
					.to_owned(),
			))?;
		}
	...
	fn instantiate_wallet(
		...
		passphrase: &str,
		account: &str,
	) -> Result<Arc<Mutex<WalletInst<LocalWalletClient, ExtKeychain>>>, libercoin_wallet::Error> {
	...
	fn execute_command(
		...
		arg_vec: Vec<&str>,
	) -> Result<String, libercoin_wallet::Error> {
	...
	/// command line tests
	fn command_line_test_impl(test_dir: &str) -> Result<(), libercoin_wallet::Error> {
		setup(test_dir);
		...
		let yml = load_yaml!("../libercoin.yml");
		let app = App::from_yaml(yml);

		// wallet init
		let arg_vec = vec!["libercoin", "wallet", "-p", "password", "init", "-h"];
		...
		let arg_vec = vec![
			"libercoin", "wallet", "-p", "password", "account", "-c", "mining",
		];
		...
		let arg_vec = vec!["libercoin", "wallet", "-p", "password", "account"];
		execute_command(&app, test_dir, "wallet2", &client2, arg_vec)?;
		...
		let wallet1 = instantiate_wallet(config1.clone(), client1.clone(), "password", "default")?;
		libercoin_wallet::controller::owner_single_use(wallet1.clone(), |api| {
		...
		let arg_vec = vec!["libercoin", "wallet", "-p", "password", "-a", "mining", "info"];
		execute_command(&app, test_dir, "wallet1", &client1, arg_vec)?;
		...
		let wallet2 = instantiate_wallet(config2.clone(), client2.clone(), "password", "default")?;
		libercoin_wallet::controller::owner_single_use(wallet2.clone(), |api| {
		...
		let arg_vec = vec!["libercoin", "wallet", "-p", "password", "check"];
		execute_command(&app, test_dir, "wallet1", &client1, arg_vec)?;
		...
		let arg_vec = vec![
			"libercoin", "wallet", "-p", "password", "-a", "mining", "cancel", "-i", "26",
		];
		execute_command(&app, test_dir, "wallet1", &client1, arg_vec)?;

		// txs and outputs (mostly spit out for a visual in test logs)
		let arg_vec = vec!["libercoin", "wallet", "-p", "password", "-a", "mining", "txs"];
		execute_command(&app, test_dir, "wallet1", &client1, arg_vec)?;

		// message output (mostly spit out for a visual in test logs)
		let arg_vec = vec![
			"libercoin", "wallet", "-p", "password", "-a", "mining", "txs", "-i", "10",
		];
		execute_command(&app, test_dir, "wallet1", &client1, arg_vec)?;

		// txs and outputs (mostly spit out for a visual in test logs)
		let arg_vec = vec![
			"libercoin", "wallet", "-p", "password", "-a", "mining", "outputs",
		];
		execute_command(&app, test_dir, "wallet1", &client1, arg_vec)?;
